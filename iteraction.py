...
class Iteraction(object):
    def __init__(self):
        ...
        self.importanceRank_path = '/Users/yuxiao/PycharmProjects/SPEC2017/'
        self.big_table_path = '/Users/yuxiao/Lab/projects/SPEC-CPU2017/508-name-d/namd_r_base.myt-result/'

        self.importanceRank_data = 'result_' + self.algorithm_name + '.xlsx'
        self.big_table_data =  self.algorithm_name + '.csv'
    ...
    def split_dataFrame(self):
        ...
        for i in range(len(sub_IR_event)):
            sub_IR_event[i] = str(sub_IR_event[i])
        #print(sub_IR_data)
        #print(sub_IR_event)

        big_table_data = pd.read_csv(big_table_data)
        print(big_table_data.columns)
        print(big_table_data['235'])
        IPC = big_table_data['235']
        big_table_data = big_table_data[sub_IR_event]
        assert len(IPC) == len(big_table_data)
        """To do"""
        big_table_data['235'] = IPC
